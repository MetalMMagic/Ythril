            ...
            <tr class="prop-row" [class.prow-open]="expandedKey() === p.key" (click)="toggleExpand(p.key)">
              ...
              <td (click)="$event.stopPropagation()">
                <div style="display:flex;gap:4px;justify-content:flex-end;">
                  <button class="btn btn-ghost btn-sm" type="button" (click)="toggleExpand(p.key)"
                    style="font-size:10px;padding:2px 8px;min-width:28px;">{{ expandedKey() === p.key ? '▲' : '▼' }}</button>
                  <button class="icon-btn danger" type="button" (click)="removeRow(p.key)" [attr.title]="'common.remove' | transloco">✕</button>
                </div>
              </td>
            </tr>
                ...
                <td colspan="4" style="padding:0;">
                  <div class="pdet">
                    <div class="pdet-head">
                      ...
                      <button class="icon-btn danger" type="button" (click)="removeRow(p.key)" [attr.title]="'spaces.schema.removePropertyTitle' | transloco">✕</button>
                    </div>
