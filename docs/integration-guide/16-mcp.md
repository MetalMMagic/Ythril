...
| `minScore` | `number` | — | Minimum cosine similarity score (0.0–1.0). Results below this threshold are excluded. **It is applied AFTER the cut to `topK`, not before** — so `topK=10, minScore=0.7` can return three even when forty records score above 0.7: the ten-record window is chosen from the unfiltered ranking and then thinned. Ask for a larger `topK` if you are filtering hard. (`find_similar` applies it inside its selection loop instead, so the same parameter behaves differently there. Tracked as `Q-3`.) |
