...
- **Delete records after (days)** — an optional space-wide expiry, **on the Danger tab** rather than here: it deletes data, so it sits with the other destructive settings. Every record (memory, entity, edge, chrono entry) created or updated in the space is deleted automatically this many days later. Leave it blank or `0` to keep records forever. Deletion propagates over sync, so an expired record won't come back from a connected peer.
...
**Retention** is the space-wide default: **Delete records after (days)**, applying to every record with no TTL of its own and no window on its type. Below the field, any type that *does* have its own window is listed read-only — that list is where you see what actually overrides this number, and it is edited on the type, in the **Schema** tab.
