...
**Settings tab:** Update the display name, purpose, usage notes for AI assistants, storage quota, auto-delete window, and document-extraction mode — grouped into **Identity**, **Purpose**, **Limits**, and **Document extraction** cards. When a storage quota, auto-delete window, or extraction override is left blank, the field's own placeholder (**Unlimited** / **No expiry**) or the **Use instance default** option shows what the default will be.
