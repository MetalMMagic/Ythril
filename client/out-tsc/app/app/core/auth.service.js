...
export class AuthService {
    constructor() {
        ...
        this._token = signal(localStorage.getItem(TOKEN_KEY), ...(ngDevMode ? [{ debugName: "_token" }] : /* istanbul ignore next */ []));
        this.token = this._token.asReadonly();
        this.isAuthenticated = computed(() => !!this._token(), ...(ngDevMode ? [{ debugName: "isAuthenticated" }] : /* istanbul ignore next */ []));
     ...
     */
    loginOidc(token, issuerUrl, clientId, scopes, idToken) {
    ...
    async logoutOidc() {
        ...
        try {
            ...
            endSessionUrl.searchParams.set('post_logout_redirect_uri', postLogoutUri ?? location.origin + '/login');
    ...
    async silentRefresh() {
        ...
        if (!issuerUrl || !clientId || !scopesStr)
            return;
        ...
        await new Promise((resolve) => {
            ...
            const handler = async (event) => {
                ...
                if (data.error || !data.code)
                    return; // Silently do nothing on failure
    ...
    async getOidcInfo() {
        try {
            return await firstValueFrom(this.http.get('/api/auth/oidc-info'));
        }
        catch {
     ...
     */
    async startOidcLogin(info) {
        if (!info.enabled || !info.issuerUrl || !info.clientId)
            return;
        // Fetch OIDC discovery document
        const discoveryUrl = info.issuerUrl.replace(/\/$/, '') + '/.well-known/openid-configuration';
        const discovery = await firstValueFrom(this.http.get(discoveryUrl));
     ...
     */
    async exchangeOidcCode(code, returnedState) {
        ...
        const tokenRes = await firstValueFrom(this.http.post(discovery.token_endpoint, body.toString(), { headers: { 'Content-Type': 'application/x-www-form-urlencoded' } }));
