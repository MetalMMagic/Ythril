  ...
  inputSchema: (s: ToolSchemas) => ({
          ...
          properties: {
            ...
            traverse: {
              ...
              ...traverseOptionSchema(MAX_RECALL_TRAVERSE),
              default: 0,
              description: `Optional graph-expansion depth (integer 0–${MAX_RECALL_TRAVERSE}, default 0). When > 0, each similar match is expanded along knowledge-graph edges up to this many hops and what the walk reached is NESTED under the match that reached it in a \`_graph\` array — {edge, node, paths} per node, identical to \`recall\`'s shape: \`edge\` is the whole edge document, \`node\` the reached entity, \`paths\` every route to it as record ids with the match first. \`count\` is the number of matches and \`graphNodes\` how many nodes were reached. A neighbourhood past the inline cap is written out in full and reported as \`graphTruncated\` + \`graphComplete\` (an authenticated download, valid one day), exactly as on \`recall\`. \`graphTruncated\` alone means a bounded link scan stopped reading, so the graph is short and no complete copy exists. With traverse > 0 the response is JSON instead of the plain text summary.`,
            },
