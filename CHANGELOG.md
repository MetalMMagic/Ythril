...
- **BREAKING (MCP): tool arguments are now validated against each tool's `inputSchema` before the handler
  ...
  unknown type, which used to be silently dropped), malformed ids, and over-cap bulk arrays. The validator
  is built per connection (the `space` `enum` is token-scoped) with a per-tool compiled cache. New standalone
  tests cover the accept path and every breaking rejection. Adds `ajv` as a server dependency.
