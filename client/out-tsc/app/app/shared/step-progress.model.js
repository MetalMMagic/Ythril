 ...
 */
export function buildBarModel(progress) {
 ...
 */
export function isStale(progressAt, stallTimeoutMs, now = Date.now()) {
    if (!progressAt)
        return false; // never reported: not the same claim as "stopped reporting"
    const t = Date.parse(progressAt);
    if (Number.isNaN(t))
        return false; // unparseable: do not accuse a job on a bad timestamp
