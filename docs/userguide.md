...
**Settings tab:** Update the display name, purpose, usage notes for AI assistants, storage quota, auto-delete window, document-extraction mode, and per-space **media-analysis** levels — grouped into **Identity**, **Purpose**, **Limits**, **Document extraction**, and **Media analysis** cards. The Media analysis card lets you override, per space, how **images**, **audio**, **video**, and **text** are analysed on upload (each defaulting to **Inherit instance default**); a level above the instance ceiling is capped to the ceiling. When a storage quota, auto-delete window, or extraction override is left blank, the field's own placeholder (**Unlimited** / **No expiry**) or the **Use instance default** / **Inherit** option shows what the default will be.
...
The **External assist model** card lets you point a bigger, hosted model at specific document tasks — currently the **document repair pass** used by the `repair` extraction level (and by `auto` when a repair model is configured). It's off unless you both fill in an **Endpoint** + **Model** *and* tick a task under **Used for**.
