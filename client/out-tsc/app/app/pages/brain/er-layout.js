 ...
 */
export const estimateLabelWidth = (r) => (r.label.length + 3 + String(r.count).length) * LABEL_CHAR_W;
const PAD = 16;
const HEAD_H = 26;
const ROW_H = 16;
/** The natural height of a box: its header plus a row per property, floored so an empty type is a box. */
const naturalHeight = (t) => HEAD_H + Math.max(2, t.properties.length + 1) * ROW_H + 12;
/**
 ...
 */
export function heightBuckets(types) {
    if (types.length === 0)
    ...
    const ceilingFor = (pred) => types.filter(t => pred(t.properties.length)).reduce((h, t) => Math.max(h, naturalHeight(t)), 0);
    const heights = [
        ceilingFor(n => n <= lo),
...
const KINDS = [
    { key: 'memories', kind: 'memory', label: 'Memories' },
    { key: 'chrono', kind: 'chrono', label: 'Chrono' },
    { key: 'files', kind: 'file', label: 'Files' },
];
/**
 ...
 * the label later.
 */
function syntheticKinds(realTypes) {
 ...
 */
export function layoutErModel(realTypes, realRels, 
/**
...
availableWidth) {
    if (realTypes.length === 0)
        return { boxes: [], paths: [], width: 0, height: 0 };
     ...
     */
    const hub = [...realTypes].sort((a, b) => degree(rels, b.type) - degree(rels, a.type) || b.count - a.count)[0];
    /**
     ...
     */
    const isolated = (t) => degree(rels, t.type) === 0;
    const left = [];
    const right = [];
    const shelf = [];
    for (const t of types) {
        ...
        if (isolated(t))
            shelf.push(t);
        else if (pointsAtHub.has(t.type))
            left.push(t);
        else
            right.push(t); // hubPointsAt, plus anything joined to something other than the hub
    }
    void hubPointsAt;
    /**
     ...
     */
    const columnOf = (t) => t === hub.type ? 1 : left.some(x => x.type === t) ? 0 : 2;
    /**
    ...
    const gapOf = (r) => {
        if (r.from === r.to)
            return null; // self-joins loop over their own box
        const cf = columnOf(r.from), ct = columnOf(r.to);
        if (cf === ct)
            return cf === 0 ? 'L' : 'R'; // same column: borrow the neighbouring gap
     ...
     */
    const widestIn = (rs) => rs.reduce((w, r) => Math.max(w, estimateLabelWidth(r)), 0);
    ...
    const gapFor = (n, widest) => Math.max(COL_GAP, 20 + n * LANE_STEP + LABEL_NUDGE + widest + 20);
    ...
    const kindFor = (name) => synth.kindOf.get(name) ?? 'entity';
    const boxes = [];
    /**
    ...
    for (let i = 0; i < rows; i++) {
        ...
        if (l)
            boxes.push({ type: l.type, x: colX[0], y: rowY, w: BOX_W, h: rowH, col: 0, kind: kindFor(l.type), count: l.count });
        if (r)
            boxes.push({ type: r.type, x: colX[2], y: rowY, w: BOX_W, h: rowH, col: 2, kind: kindFor(r.type), count: r.count });
        rowY += rowH + GAP_Y;
    }
    ...
    for (const r of rels) {
        ...
        const faceRight = (box) => box.x + box.w;
