import { ChangeDetectionStrategy, Component, computed, input } from '@angular/core';
import * as i0 from "@angular/core";
/**
 * The Ythril brand lockup — the orb-with-glowing-Y mark standing in for the capital **Y**, followed by
 * "thril", so it reads **"Ythril"**. One source of truth for the wordmark: the top-bar and every pre-auth
 * screen (login / setup / OIDC callback) render this instead of hand-inlining the SVG or an old text dot.
 *
 * `size` is the mark's height in px; the "thril" text scales with it (matched to the 21px header at 16px).
 * The visible mark + word are decorative (`aria-hidden`); a visually-hidden "Ythril" carries the accessible
 * name, so a screen reader always hears the brand, whatever wraps the component.
 */
export class BrandLogoComponent {
    constructor() {
        /** Mark height in px; the wordmark text scales with it. */
        this.size = input(21, ...(ngDevMode ? [{ debugName: "size" }] : /* istanbul ignore next */ []));
        this.wordPx = computed(() => Math.round(this.size() * 0.76), ...(ngDevMode ? [{ debugName: "wordPx" }] : /* istanbul ignore next */ []));
    }
    static { this.ɵfac = function BrandLogoComponent_Factory(__ngFactoryType__) { return new (__ngFactoryType__ || BrandLogoComponent)(); }; }
    static { this.ɵcmp = /*@__PURE__*/ i0.ɵɵdefineComponent({ type: BrandLogoComponent, selectors: [["app-brand-logo"]], inputs: { size: [1, "size"] }, decls: 21, vars: 6, consts: [[1, "bl"], ["viewBox", "0 0 32 32", "aria-hidden", "true", "focusable", "false", 1, "bl-mark"], ["id", "blOrb", "cx", "42%", "cy", "36%", "r", "72%"], ["offset", "0%", "stop-color", "#1a1f26"], ["offset", "58%", "stop-color", "#0a0c0f"], ["offset", "100%", "stop-color", "#040507"], ["id", "blHalo", "cx", "50%", "cy", "47%", "r", "50%"], ["offset", "0%", "stop-opacity", "0.55", 1, "bl-c"], ["offset", "55%", "stop-opacity", "0.12", 1, "bl-c"], ["offset", "100%", "stop-opacity", "0", 1, "bl-c"], ["cx", "16", "cy", "16", "r", "15.5", "fill", "url(#blOrb)"], ["cx", "16", "cy", "16", "r", "15.3", "fill", "none", "stroke-opacity", "0.16", "stroke-width", "0.7", 1, "bl-ring"], ["cx", "16", "cy", "15.2", "r", "9.5", "fill", "url(#blHalo)"], [1, "bl-glyph"], ["d", "M5.8 6 L16 19 L26.2 6 L23.1 7.7 L16 14.6 L8.9 7.7 Z"], ["d", "M13.3 13.6 L18.7 13.6 L16 27 Z"], ["aria-hidden", "true"], [1, "bl-sr"]], template: function BrandLogoComponent_Template(rf, ctx) { if (rf & 1) {
            i0.ɵɵdomElementStart(0, "span", 0);
            i0.ɵɵnamespaceSVG();
            i0.ɵɵdomElementStart(1, "svg", 1)(2, "defs")(3, "radialGradient", 2);
            i0.ɵɵdomElement(4, "stop", 3)(5, "stop", 4)(6, "stop", 5);
            i0.ɵɵdomElementEnd();
            i0.ɵɵdomElementStart(7, "radialGradient", 6);
            i0.ɵɵdomElement(8, "stop", 7)(9, "stop", 8)(10, "stop", 9);
            i0.ɵɵdomElementEnd()();
            i0.ɵɵdomElement(11, "circle", 10)(12, "circle", 11)(13, "circle", 12);
            i0.ɵɵdomElementStart(14, "g", 13);
            i0.ɵɵdomElement(15, "path", 14)(16, "path", 15);
            i0.ɵɵdomElementEnd()();
            i0.ɵɵnamespaceHTML();
            i0.ɵɵdomElementStart(17, "span", 16);
            i0.ɵɵtext(18, "thril");
            i0.ɵɵdomElementEnd();
            i0.ɵɵdomElementStart(19, "span", 17);
            i0.ɵɵtext(20, "Ythril");
            i0.ɵɵdomElementEnd()();
        } if (rf & 2) {
            i0.ɵɵadvance();
            i0.ɵɵstyleProp("width", ctx.size(), "px")("height", ctx.size(), "px");
            i0.ɵɵadvance(16);
            i0.ɵɵstyleProp("font-size", ctx.wordPx(), "px");
        } }, styles: [".bl[_ngcontent-%COMP%] { display: inline-flex; align-items: center; gap: 2px; font-weight: 700; letter-spacing: -0.03em;\n      line-height: 1; color: var(--text-primary); }\n    .bl-mark[_ngcontent-%COMP%] { flex-shrink: 0; display: block; }\n\n    \n\n\n\n\n\n\n\n\n\n\n\n    .bl-mark[_ngcontent-%COMP%] { --bl: var(--brand-mark, var(--accent, #9eec55)); }\n    .bl-glyph[_ngcontent-%COMP%] { fill: var(--bl); }\n    .bl-ring[_ngcontent-%COMP%]  { stroke: var(--bl); }\n    .bl-c[_ngcontent-%COMP%]     { stop-color: var(--bl); }\n    .bl-sr[_ngcontent-%COMP%] { position: absolute; width: 1px; height: 1px; padding: 0; margin: -1px; overflow: hidden;\n      clip: rect(0, 0, 0, 0); white-space: nowrap; border: 0; }"], changeDetection: 0 }); }
}
(() => { (typeof ngDevMode === "undefined" || ngDevMode) && i0.ɵsetClassMetadata(BrandLogoComponent, [{
        type: Component,
        args: [{ selector: 'app-brand-logo', standalone: true, changeDetection: ChangeDetectionStrategy.OnPush, template: `
    <span class="bl">
      <svg class="bl-mark" [style.width.px]="size()" [style.height.px]="size()" viewBox="0 0 32 32" aria-hidden="true" focusable="false">
        <defs>
          <radialGradient id="blOrb" cx="42%" cy="36%" r="72%">
            <stop offset="0%" stop-color="#1a1f26"/><stop offset="58%" stop-color="#0a0c0f"/><stop offset="100%" stop-color="#040507"/>
          </radialGradient>
          <radialGradient id="blHalo" cx="50%" cy="47%" r="50%">
            <stop class="bl-c" offset="0%" stop-opacity="0.55"/><stop class="bl-c" offset="55%" stop-opacity="0.12"/><stop class="bl-c" offset="100%" stop-opacity="0"/>
          </radialGradient>
        </defs>
        <circle cx="16" cy="16" r="15.5" fill="url(#blOrb)"/>
        <circle class="bl-ring" cx="16" cy="16" r="15.3" fill="none" stroke-opacity="0.16" stroke-width="0.7"/>
        <circle cx="16" cy="15.2" r="9.5" fill="url(#blHalo)"/>
        <g class="bl-glyph">
          <path d="M5.8 6 L16 19 L26.2 6 L23.1 7.7 L16 14.6 L8.9 7.7 Z"/>
          <path d="M13.3 13.6 L18.7 13.6 L16 27 Z"/>
        </g>
      </svg><span aria-hidden="true" [style.font-size.px]="wordPx()">thril</span>
      <span class="bl-sr">Ythril</span>
    </span>
  `, styles: ["\n    .bl { display: inline-flex; align-items: center; gap: 2px; font-weight: 700; letter-spacing: -0.03em;\n      line-height: 1; color: var(--text-primary); }\n    .bl-mark { flex-shrink: 0; display: block; }\n\n    /*\n     * The mark's colour is a THEME TOKEN, not a constant.\n     *\n     * #9eec55 was hard-coded in five places, so a theme could restyle the entire product and the logo\n     * stayed green. --brand-mark overrides it directly; absent that it follows --accent, so a theme\n     * that only sets an accent gets a matching mark for free \u2014 which is what an operator actually wants.\n     *\n     * These are CSS properties rather than SVG attributes on purpose: fill=\"var(--x)\" in an attribute\n     * does not resolve. As CSS they do, and they beat shipping one file per colour \u2014 any hex works, not\n     * a fixed palette somebody has to extend.\n     */\n    .bl-mark { --bl: var(--brand-mark, var(--accent, #9eec55)); }\n    .bl-glyph { fill: var(--bl); }\n    .bl-ring  { stroke: var(--bl); }\n    .bl-c     { stop-color: var(--bl); }\n    .bl-sr { position: absolute; width: 1px; height: 1px; padding: 0; margin: -1px; overflow: hidden;\n      clip: rect(0, 0, 0, 0); white-space: nowrap; border: 0; }\n  "] }]
    }], null, { size: [{ type: i0.Input, args: [{ isSignal: true, alias: "size", required: false }] }] }); })();
(() => { (typeof ngDevMode === "undefined" || ngDevMode) && i0.ɵsetClassDebugInfo(BrandLogoComponent, { className: "BrandLogoComponent", filePath: "app/shared/brand-logo.component.ts", lineNumber: 62 }); })();
